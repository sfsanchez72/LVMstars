...
def test_mask_bad_pixels():
    ...
    assert mask[0]  # Good pixel (SNR=10)
    assert mask[1]  # Good pixel (SNR=10)
    assert not mask[2]  # Bad pixel (inf)
    assert mask[3]  # Good pixel (SNR=10)
    assert not mask[4]  # Bad pixel (nan)
...
def test_calculate_snr_with_wavelength_range():
    ...
    snr = calculate_snr(flux, flux_error, wavelength_range=(4500, 5500), wavelength=wavelength)
